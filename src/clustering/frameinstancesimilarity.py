...
def frame_element_relatedness(fe1, fe2, roles=False, fesim='wup', cache={}):
    ...
    for s1 in fe1:
        ...
        for s2 in fe2:
            if roles == True and s1.role != s2.role:
                ...
                sim = synset_similarity(s1.entity, s2.entity, fesim=fesim, cache=cache)
            if sim > max_sim:
                max_sim = sim
...
def frame_instance_similarity(fi1, fi2, alpha=0.5, roles=False, ftsim='occ', fesim='wup', cache_frames={}, cache_synsets={}):
    # the alpha prameter will be read from a config file
    frame_sim = frame_relatedness(fi1.frame_type, fi2.frame_type, ftsim=ftsim, cache=cache_frames)
    fe_sim = frame_element_relatedness(fi1.frame_elements, fi2.frame_elements, roles=roles, fesim=fesim, cache=cache_synsets)
    sim = alpha * frame_sim + (1.0-alpha) * fe_sim
    return sim

def get_from_cache(item1, item2, cache):
    if item1 == item2:
        return 1.0
