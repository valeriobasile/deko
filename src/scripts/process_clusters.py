...
print "</head><body>"
print "<h1>Clusters from file: {0}</h1>".format(options.cluster_file)
print "<table>"
print "<thead><th>Cluster id</th><th># frame instances</th><th>Frame types</th><th>Elements with role</th><th>Elements without roles</th></thead>"
for cluster_id, instances in clusters_sorted:
    ...
    print "<tr>"
    print "<td>{0}</td><td>{1}</td>".format(cluster_id, len(instances))
    print "<td>"
    for cluster_instance_type in cluster_instance_types_count:
        count = cluster_instance_type[1]
        max_count = max([x[1] for x in cluster_instance_types_count])
        color = show_color(count, max_count)
        print '<span style="color:{0}">{1} ({2})</span><br/>'.format(color, cluster_instance_type[0], count)
    print "</td>"
    print "<td>"
    for cluster_elements_role in cluster_elements_roles_count:
        count = cluster_elements_role[1]
        max_count = max([x[1] for x in cluster_elements_roles_count])
        color = show_color(count, max_count)
        print '<span style="color:{0}"><em>{1}</em> {2} ({3})</span><br/>'.format(color, cluster_elements_role[0][0], cluster_elements_role[0][1], count)
    print "</td>"
    print "<td>"
    for cluster_element in cluster_elements_count:
        count = cluster_element[1]
        max_count = max([x[1] for x in cluster_elements_count])
        color = show_color(count, max_count)
        print '<span style="color:{0}">{1} ({2})</span><br/>'.format(color, cluster_element[0], count)
    print "</td>"
    #print cluster_instance_types_count, cluster_elements_roles_count, cluster_elements_count
    print "</tr>"
